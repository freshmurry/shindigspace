class VenuesController < ApplicationController
  ...
  def create
    ...
    if @venue.save
      redirect_to listing_venue_path(@venue), notice: "Venue was successfully created."
    else
  ...
  def show
    ...
    @photos = @venue.photos
    @guest_reviews = Review.where(type: "GuestReview", venue_id: @venue.id)
    @nearby_venues = @venue.nearbys(10) if @venue.geocoded?

    # Ensure latitude and longitude are populated
    if @venue.latitude.nil? || @venue.longitude.nil?
      Rails.logger.warn "Venue coordinates are not available for Venue ID: #{@venue.id}"
    end
  end
  ...
  def destroy
    ...
    redirect_to venue_path, notice: "Venue was successfully deleted."
  end
